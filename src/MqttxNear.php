...
class MqttxNear
{
    ...
    /**
     * host unix time, used to detect disconnects
     *
     * @var int
     */
     ...
     * @param bool $debug
     */
    public function __construct($address, $port, $clientId, $cafile = null, $debug = false)
    {
        $this->debug = $debug;
     ...
     * @return bool
     */
    public function reconnect($clean = true, $will = null, $username = null, $password = null): bool
    {
    ...
    public function connect($clean = true, $will = null, $username = null, $password = null): bool
    {
        ...
        if (!$this->socket) {
            $this->__debugx("stream_socket_create() $errno, $errstr", 'error');
            return false;
        }
        ...
        if (ord($string{0}) >> 4 === 2 && $string{3} === chr(0)) {
            $this->__debugx('Connected to Broker', 'debug');
        } else {
            $this->__debugx(
                sprintf(
        ...
        foreach ($this->topics as $key => $top) {
            if (preg_match(
                ...
                if (is_callable($top['function'])) {
                    ...
                    $this->__debugx('Message received on topic ' . $topic . ' but function is not callable.', 'error');
                }
            }
        }

        if ($found === false) {
            $this->__debugx('msg received but no match in subscriptions', 'debug');
        }
    ...
    {
        if (feof($this->socket)) {
            $this->__debugx('eof receive going to reconnect for good measure', 'debug');
            fclose($this->socket);
        ...
        if ((string)$byte === '') {
            ...
            $this->__debugx(
                sprintf(
                    'Received CMD: %d (%s)',
                    $cmd,
                    isset(static::$commands[$cmd]) === true ? static::$commands[$cmd] : 'Unknown'
                ),
                'info'
            );
            ...
            $this->__debugx('Fetching: ' . $value . ' bytes', 'debug');
        ...
        if ($this->timesinceping < (time() - $this->keepalive)) {
            $this->__debugx('not had something in a while so ping', 'debug');
            $this->ping();
        }

        if ($this->timesinceping < (time() - ($this->keepalive * 2))) {
            $this->__debugx('not seen a packet in a while, disconnecting/reconnecting', 'debug');
            fclose($this->socket);
            $this->reconnect(false);
            if (count($this->topics)) {
                $this->subscribe($this->topics);
